...
public class Titles {
	...
	public static void titleOnPlayerJoin(Player player) {
		...
		Bukkit.getScheduler().scheduleSyncDelayedTask(Main.getInstance(), new Runnable() {
			@Override
			public void run() {
				BountifulAPI.sendTitle(player, 5, 60, 5, ChatColor.GOLD + "Welcome back, " + player.getName() + "!",
						"");
			}
		}, 100L);
		...
		if (MailPlaceholder.buildsString(player) != null) {
			if (addDelay) {
				Bukkit.getScheduler().scheduleSyncDelayedTask(Main.getInstance(), new Runnable() {
					...
					public void run() {
						if (MailPlaceholder.buildsString(player) != null) {
							BountifulAPI.sendTitle(player, 5, 80, 5,
									ChatColor.RED + MailPlaceholder.buildsString(player),
